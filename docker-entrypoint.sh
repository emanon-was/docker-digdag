...
if [ -n "$SERVER_ACCESS_LOG_PATTERN" ]; then
    case "$SERVER_ACCESS_LOG_PATTERN" in
        json | combined | common ) echo "server.access-log.pattern = $SERVER_ACCESS_LOG_PATTERN" >> $DIGDAG_CONFIG;
        * ) echo "Invalid parameter: SERVER_ACCESS_LOG_PATTERN = [json, combined, common]"; exit 1;
    esac
fi
...
if [ -n "$DATABASE_TYPE" ]; then
    case "$DATABASE_TYPE" in
        h2 | postgresql ) echo "database.type = $DATABASE_TYPE" >> $DIGDAG_CONFIG;
        * ) echo "Invalid parameter: DATABASE_TYPE = [h2, postgresql]"; exit 1;
    esac
fi
...
if [ -n "$ARCHIVE_TYPE" ]; then
    case "$ARCHIVE_TYPE" in
        db | s3 | gcs ) echo "archive.type = $ARCHIVE_TYPE" >> $DIGDAG_CONFIG;
        * ) echo "Invalid parameter: ARCHIVE_TYPE = [db, s3, gcs]"; exit 1;
    esac
fi
...
if [ -n "$LOG_SERVER_TYPE" ]; then
    case "$LOG_SERVER_TYPE" in
        local | null | s3 | gcs ) echo "log-server.type = $LOG_SERVER_TYPE" >> $DIGDAG_CONFIG;
        * ) echo "Invalid parameter: LOG_SERVER_TYPE = [local, null, s3, gcs]"; exit 1;
    esac
fi
